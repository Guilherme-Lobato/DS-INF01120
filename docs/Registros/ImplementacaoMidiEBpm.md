...
- **Ação Realizada:**
  - Foi modificado o método `gerar_timeline` do `PolifoniaService` para realizar uma segunda iteração cronológica na lista de eventos já ordenada.
  - Utilizou-se a classe `ContextoGlobal` para rastrear as mudanças temporais e injetar a propriedade `bpm` exata em todos os eventos do tipo `CHANGE_BPM`, comunicando a alteração para a engine de áudio no momento correto.
...
- **Ação Realizada:**
  - Foi instalada a biblioteca `midiutil` no ambiente backend (`requirements.txt`).
  - Foi criada a classe `MidiExporter`, que constrói um arquivo com `N` pistas de áudio (Tracks), onde cada `Voz` possui seu próprio instrumento num canal (Channel) exclusivo (ignorando o canal 9 para evitar percussão).
  ...
  - A classe `InterfaceController` foi atualizada para orquestrar a chamada do serviço e adicionar o `midi_base64` ao retorno final.